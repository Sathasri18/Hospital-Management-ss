	...
	class modify implements ActionListener
	{
		public void actionPerformed(ActionEvent ae)
		{
			try{
				...
				String str="UPDATE PAT SET name=?,addr=?,contact=?,hist=?,blgr=?,dob=?,current=?,room=?,dateadd=?,rtype=?,gender=?,docname=? WHERE num=?";
	...
	class submit implements ActionListener
	{
		...
		{			
			try{
				...
				ResultSet rs=st.executeQuery("SELECT * FROM PAT WHERE num="+num);
